...
[ ! -f /tmp/.initialized ] && echo "Configuring default settings" && curl -XPUT $ELASTICSEARCH_URL/.kibana/config/4.5.0 -d '{"dashboard:defaultDarkTheme": true, "defaultIndex": "request-*"}' && touch /tmp/.initialized
...
DIR=.
echo "Loading dashboards to ${ELASTICSEARCH} in ${KIBANA_INDEX} from"
pwd
